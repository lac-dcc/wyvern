...
Function *cloneCalleeFunction(Function &Callee, int index, Function &thunk, PointerType *fPtrType, Module &M) {
	...
	std::string functionName = "_wyvern_calleeclone_" + Callee.getName().str() + "_" + std::to_string(index);
	Function *newCallee = Function::Create(FT, Function::ExternalLinkage, functionName, M);
	...
	CloneFunctionInto(newCallee, &Callee, vMap, CloneFunctionChangeType::LocalChangesOnly, Returns);
...
void WyvernLazyficationPass::lazifyCallsite(CallInst &CI, int index, Module &M) {
	...
	errs() << "Lazifying: " << *lazyfiableArg << " in func " << caller->getName() << " call to " << callee->getName() << "\n";
	...
	Function *newCallee = cloneCalleeFunction(*callee, index, *thunk, functionPtrType, M);
...
static RegisterPass<WyvernLazyficationPass> X("lazify-callsites", "Wyvern - Lazify function arguments for callsites deemed optimizable.", false, false);
