...
struct FindLazyfiableAnalysis : public ModulePass {
	...
	void DFS(BasicBlock *first, BasicBlock *exit, std::set<BasicBlock*> &visited, Value *arg, int index) {
		...
		while(!st.empty()) {
			...
			for (auto it = succ_begin(cur), it_end = succ_end(cur); it != it_end; ++it) {
				...
				if (visited.count(succ) == 0) {
					visited.insert(succ);
					st.push(succ);
					//DFS(succ, exit, visited, arg, index);
				}
			}
		}
	}

	void findLazyfiablePaths(Function &F) {
	...
	bool isArgumentComplex(Instruction *I, std::set<Instruction*> analyzed, int depth) {
		if (depth > 12) {
			return false;
		}
